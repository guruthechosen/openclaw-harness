//! Moltbot (formerly Clawdbot) log collector
//!
//! Monitors:
...
/// Collector for Moltbot/Clawdbot
pub struct MoltbotCollector {
...
impl MoltbotCollector {
    ...
    fn parse_log_line(&self, line: &str) -> Vec<AgentAction> {
        ...
        let entry: MoltbotLogEntry = match serde_json::from_str(line) {
            Ok(e) => e,
...
#[async_trait]
impl super::Collector for MoltbotCollector {
    fn name(&self) -> &'static str {
        "moltbot"
    }

    async fn start(&self, tx: mpsc::Sender<AgentAction>) -> anyhow::Result<()> {
        info!("🦞 Starting Moltbot collector, watching: {:?}", self.sessions_dir);

        if !self.sessions_dir.exists() {
            warn!("Moltbot sessions directory not found: {:?}", self.sessions_dir);
            return Ok(());
        }
        ...
        info!("Moltbot collector started, monitoring for new tool calls...");
    ...
    async fn stop(&self) -> anyhow::Result<()> {
        info!("Stopping Moltbot collector");
        Ok(())
    }
...
// ============================================
// Serde structures for parsing Moltbot logs
// ============================================

#[derive(Debug, Deserialize)]
struct MoltbotLogEntry {
    #[serde(rename = "type")]
    entry_type: String,
    id: String,
    timestamp: String,
...
mod tests {
    ...
    fn test_parse_exec_log() {
        ...
        assert_eq!(action.agent, AgentType::Moltbot);
