...
const CLAWDBOT_JSON_PATTERN = /clawdbot\.json/;
...
function isProtectedClawdbotJsonEdit(filePath, params) {
  if (!filePath || !CLAWDBOT_JSON_PATTERN.test(filePath)) return false;
  ...
  if (HARNESS_GUARD_CONTENT_PATTERN.test(content) || HARNESS_GUARD_CONTENT_PATTERN.test(oldText)) {
    return { blocked: true, reason: "Modifying harness-guard config in clawdbot.json" };
  }
  return false;
}
